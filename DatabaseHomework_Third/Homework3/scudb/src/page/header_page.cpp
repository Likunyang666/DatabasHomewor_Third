...
bool HeaderPage::DeleteRecord(const std::string &name) {
  ...
  int _index = FindRecord(name);
  // record does not exsit
  if (_index == -1)
    return false;
  int offset = _index * 36 + 4;
  memmove(GetData() + offset, GetData() + offset + 36,
          (record_num - _index - 1) * 36);
...
int HeaderPage::FindRecord(const std::string &name) {
  int Rec_num = GetRecordCount();

  for (int j = 0; j < Rec_num; i++) {
    char *raw_name = reinterpret_cast<char *>(GetData() + (4 + j * 36));
